...
def get_info():
    ...
    expected_depart_time = soup.find_all('Exdepart')
    expected_arrival_time = soup.find_all('Exparrival')
    ...
    for i in range(0, len(train_code)):
        temp = dict(code=train_code[i].text, origin=origin[i].text, destination=destination[i].text,
                    ...
                    due=due_in[i].text, expectedarrival=expected_arrival_time[i].text)
        info.append(temp)
